...
Simulation::GenBlock::GenBlock(Simulation& s, Block* next, size_t priority): Block(s, next) {}
Simulation::AdvanceBlock::AdvanceBlock(Simulation& s, Block* next, RandomGenerator gen): Block(s, next), gen(gen) {}
...
Simulation::Block* Simulation::QueueBlock::advance(priority_t) {
    ++sim.queues[q_index].data;
    return next;
}

Simulation::Block* Simulation::DepartBlock::advance(priority_t) {
    if (sim.queues[q_index].data == 0) throw SimulationException("Attempted to leave empty queue");
    --sim.queues[q_index].data;
    return next;
}

Simulation::Block* Simulation::EnterBlock::advance(priority_t pr) {
    if ( sim.storages[storage_index].data->enter(pr, this)) return next;
    return nullptr; 
}

Simulation::Block* Simulation::LeaveBlock::advance(priority_t) {
    sim.storages[storage_index].data->leave();
    return next;
}

Simulation::Block* Simulation::GenBlock::advance(priority_t) {
    return next;
}

Simulation::Block* Simulation::AdvanceBlock::advance(priority_t pr) {
...
Simulation::Block* Simulation::GateBlock::advance(priority_t pr) {
    if ((q.empty() || (pr > q.top())) && expr.eval()) return next;
    q.push(pr); return nullptr; // check will be conducted in the end of tick
}

Simulation::Block* Simulation::TransferBlock_imm::advance(priority_t) {
    return sim.labels[index].data;
}

Simulation::Block* Simulation::TransferBlock_expr::advance(priority_t) {
    if (!expr.eval()) return next;
    return sim.labels[alt_index].data;
}

Simulation::Block* Simulation::TransferBlock_prob::advance(priority_t) {
    if (dist(gen) < prob) return sim.labels[alt_index].data;
    return next;
}

Simulation::Block* Simulation::TerminateBlock::advance(priority_t) { return nullptr; }
...
bool Simulation::Storage::enter(priority_t pr, EnterBlock* ret) {
